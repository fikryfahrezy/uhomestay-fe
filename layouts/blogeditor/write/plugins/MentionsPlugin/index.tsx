...
const dummyLookupService = {
  search(string: string, callback: (results: Array<string> | null) => void) {
    setTimeout(() => {
...
type MentionsTypeaheadItemProps = {
  index: number;
  isHovered: boolean;
  isSelected: boolean;
  onClick: () => void;
...
function MentionsTypeaheadItem({
  ...
  onMouseLeave,
  result,
}: MentionsTypeaheadItemProps) {
  const liRef = useRef(null);

  let className = "item";
  if (isSelected) {
    className += " " + styles["li-selected"];
  }
  if (isHovered) {
    className += " " + styles["li-hovered"];
  }
...
function MentionsTypeahead({
  ...
  useEffect(() => {
    ...
    if (results !== null && div !== null && rootElement !== null) {
      ...
      div.style.top = `${top - height - divHeight}px`;
  ...
  return (
    <div
      ...
      role="listbox"
      ref={divRef}
      className={styles["mentions-typeahead"]}
    >
      <ul>
...
function tryToPositionRange(match: MentionMatch, range: Range): boolean {
  ...
  try {
    range.setStart(anchorNode as Node, startOffset);
    range.setEnd(anchorNode as Node, endOffset);
  } catch (error) {
    return false;
  }
...
function useMentions(editor: LexicalEditor): JSX.Element | null {
